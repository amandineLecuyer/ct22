...
class cp_feedback{
	...
	private $plugin_url = EBEC_URL;
	private $plugin_version = EBEC_VERSION;
	private $plugin_name = 'Events Block For The Events Calendar';
	private $plugin_slug = 'ebec';
	...
	*/
	...
	function submit_deactivation_response() {
		if (!isset($_POST['_wpnonce']) || !wp_verify_nonce(sanitize_text_field($_POST['_wpnonce']), '_cool-plugins_deactivate_feedback_nonce')) {
			...
			$allowed_urls = array('http://feedback.coolplugins.net/wp-json/coolplugins-feedback/v1/feedback');
			if (!in_array($feedback_url, $allowed_urls)) {
				wp_send_json_error('Invalid feedback URL');
				exit;
			}
