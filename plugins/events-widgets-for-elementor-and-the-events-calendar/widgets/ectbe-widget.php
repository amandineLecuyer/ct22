		...
		if ( $layout == 'calendar' ) {
			...
			if ( ! empty( $all_events ) ) {
				foreach ( $all_events as $event ) {
					...
					$ectbe_read_more = '<div class="ectbe-evt-more-box"><a class="ectbe-evt-read-more" href="' . esc_url( $url ) . '">' . esc_html__( "Voir plus d'informations", 'ectbe' ) . '</a></div>';
				...
				$event_output .= '<h3>' . esc_html__( "Aucun évènements à venir", 'ectbe' ) . '</h3>';
			}
